...
% Pressure
P = 68;             % atm
% Temperature, can be range or single value
%T0 = 298;           % K, desired initial temperature

% Initial Temperature if using equivalency principle 
q_bar = 140                 %W/cm^2
D.rhoc  = 1780/(100^3); 
r_b = 1.597;
D.cp    = 1.39*1000;   
...
for kk = 1:length(T0)
    for ii = 1:length(P)
        ...
        D.cp    = 1.39*1000;                                      % J/kg-K,      condensed phase specific heat
        D.kg    = 0.086;                                          % W/m-K,       gas phase conductivity
        D.Ac    = 1.426e15;                                      % 1/s,         Arrhenius pre-factor
        D.kc    = 0.2;                                           % W/m-K,       condensed phase conductivity
        D.Ec    = 170*1000;                                      % J/mol,       condensed phase activation energy
        D.Eg    = 136.07*1000;                                      % J/mol,       gas phasae activiation energy (for williams model)
        D.rhoc  = 1780;                                          % kg/m^3,      condensed phase density
        D.MW    = 34.2/1000;                                     % kg/mol,     condensed phase molecular weight
        D.Bg    = 3.275e-3;                                        % m^3/kg-s-K^2,Arrhenius pre-factor
        D.BgW   = 51.97;                                           % m^3/kg-s-K^2,Arrhenius pre-factor for williams model
...
%Sensitivity Analysis (derivative of xgd with respect to T or P)
% sens_wsb = (diff(xgd)./diff(T0))./xgd(1:length(xgd)-1);
% sens_wdb = (diff(xgd_DB)./diff(T0))./xgd_DB(1:length(xgd_DB)-1);
...
figure(2);
plot(T0,xgd,'r',T0_cels,xgd_DB,'b');
title('ME 697C, HW 3, Problem 3, RDX - Allison Murray');
